...
public class RequestManager {
	...
	// 取运行内存阈值的1/3作为图片缓存
	private static final int MEM_CACHE_SIZE = 1024 * 1024 * ((ActivityManager) Driibo
	...
	private static Cache openCache() {
		return new DiskBasedCache(CacheUtils.getExternalCacheDir(Driibo
					...
					if (!isImmediate && defaultImageDrawable != null) {
						TransitionDrawable transitionDrawable = new TransitionDrawable(
								new Drawable[] {
										...
										new BitmapDrawable(Driibo.getContext()
												.getResources(),
												response.getBitmap()) });
